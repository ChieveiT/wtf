...
class Router
{
    ...
    private $host = null;
    private $service_environment = null;
    private $middleware = [];
    ...
    public function host($host, Closure $callback, $not_found)
    {
        $this->host = $host;
        $callback();
        $this->host = null;
    ...
    {
        if (isset($statements['service_environment'])) {
    ...
    public function dispatch($host, $method, $uri)
    {
        //数据完整性和合法性
        if (!($host && in_array($method, ['GET', 'POST', 'PUT', 'DELETE']) && $uri)) {
            throw new Exception("Illegal Request: Router can't accept the fragmentary request.");
        }
        ...
        $static_uri_map = $type_map['static'];
        if (isset($static_uri_map[$uri])) {
            $matched_route = $static_uri_map[$uri];
        }
        ...
        if (!isset($matched_route)) {
            throw new NotFoundException("Not Match: Router can't find the route to match the request.");
        }
        
        //把host暂存起来，$this->url()在路由缺省host的情况下会填写当前请求的host
